...
[ILLUSTRATION: A dramatic close-up of a security camera lens, with its field of view filled by a busy school hallway. Overlaid on the scene: colored bounding boxes around detected people, labels like "Student (96%)," "Backpack (88%)," "Door (91%)." The data overlays look like a live HUD screen from a spy movie. Exciting, a bit futuristic, but grounded in a real school setting.]
...
> - 🔓 *Unlocking now (Part 1):* Vision — the ability to process and describe images
>
> Up until this moment, your agent was like someone who could talk but couldn't *see.* It could answer questions typed at it, but hand it a photo? Nothing.
>
> That changes in this chapter. By the time you finish Lab 2.4, your agent will be able to take a photo and describe what it sees.
>
> Chapter 4 is next in this module: *Hearing.* Once your agent can see, you'll give it ears — the ability to listen to audio and transcribe speech.
...
We'll talk more about this in Section 5 on bias. For now, keep this in your back pocket: **data shapes what AI can see — and what it can't.**
...
Some of the models relevant to this chapter include:
...
*(For speech recognition models like Whisper, see Chapter 4.)*
...
## Section 5: Bias in Computer Vision — When AI Gets It Wrong Systematically

Here's an uncomfortable truth about AI vision: **it doesn't fail equally for everyone.**

Some people are much more likely to be misidentified by face recognition systems than others. These failures aren't random glitches — they're patterns. Systematic patterns that trace back to a single root cause: **the training data.**
...
Acknowledging these problems isn't the same as saying AI vision is broken beyond repair. It's not. But fixing it requires:
...
## Section 6: Your Agent's New Eyes — What You Built in Lab 2.4 (Vision Half)

Let's zoom out and look at what you actually built in the vision portion of Lab 2.4.

You connected your OpenClaw agent to an image captioning model on Hugging Face:

**An image captioning model** (like `Salesforce/blip-image-captioning-base`) — Give it an image, it produces a sentence describing what it sees.
...
> ### 🤖 Your Agent's Journey — Half an Eye
>
> You've unlocked vision. Your agent now has:
>
> - **Module 1 unlock:** Identity — name, personality, memory
> - **Module 2, Chapter 3:** Vision — image captioning
>
> But perception is more than just seeing. In **Chapter 4**, you'll give your agent the ability to *hear* — to receive an audio clip and understand what was said. Vision and hearing together make up your agent's full perceptual toolkit.
>
> See you there.
...
**Bias is baked into vision AI.** Because training data over-represents some groups and under-represents others, AI vision systems perform unevenly across different people. The Gender Shades study showed error rates up to 35× higher for darker-skinned women. This has real consequences in policing, healthcare, and everyday life.

**Your agent can now see.** By connecting to Hugging Face models through an API, your agent can describe images — capabilities that would have required millions of dollars of compute power a decade ago.
...
| **Gender Shades** | A 2018 study by Joy Buolamwini that revealed large accuracy gaps in facial recognition systems across demographic groups |
...
**3.** Why does the quality and diversity of training data matter so much for AI vision? Give one real-world example of what happens when training data is not representative.

*(Guidance: Look for connection between training data → model capability. Example could be facial recognition accuracy gaps from Gender Shades study, self-driving car failures, or any other example from the chapter.)*

---

**4.** What is the difference between image classification, object detection, and image captioning? Give an example of a situation where you'd want each.

*(Guidance: Classification — label entire image, good for sorting photos. Detection — find and locate multiple objects, needed for self-driving cars or security cameras. Captioning — natural language description, useful for accessibility or describing images to visually impaired users.)*
...
**🤗 Hugging Face Spaces** — Interactive demos of hundreds of AI vision models you can try in your browser right now. No coding required.
...
**🔬 Teachable Machine by Google** — Train your own image classification model in minutes, directly in your browser. Excellent preview of Module 4 topics.
...
**🎥 "Coded Bias"** — A documentary following Joy Buolamwini's investigation into AI bias in facial recognition systems. Available on Netflix and PBS.
[QR CODE: "Coded Bias" documentary]
...
*Note: This chapter was previously the first half of chapter-2.md (combined "How AI Sees and Hears"). Split into Chapter 3 (vision) and Chapter 4 (audio/speech) per canonical chapter plan.*
